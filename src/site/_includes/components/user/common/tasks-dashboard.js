...
if (!(days || months || years)) {
  dv.el(
    "strong",
    "Task Graphs: to generate graphs, please add days, months, or years (or all three!) in your file properties."
  );
}
...
function countTasks(dates) {
  ...
  for (let i = dates.length - 1; i >= 0; i--) {
    ...
    let tasks = dv
      .pages(projectFolder)
      .file.tasks.where((t) => t.text.includes(dString));
...
if (!window.renderChart) {
  dv.el(
    "strong",
    "Task Graphs: Please install and/or enable the Obsidian Charts plugin"
  );
} else if (!input.container) {
  dv.el(
    "strong",
    "Task Graphs: Your include has a syntax error. Please see documentation and supply the correct parameters."
  );
} else {
